...
def train(args):
    ...
    if args.ckpt_path:
        ...
        full_experiment_name_original = args.get_ckpt_path_from_experiment_name + \
            "-seed-" + str(args.seed)
        experiment_id_original = sha1(
            full_experiment_name_original.encode("utf-8")).hexdigest()[:8]
        checkpoint_dir_wandb = os.path.join(
            fulldir_experiments_for_ckpt_path, "lag-llama", experiment_id_original, "checkpoints")
        ...
        if not args.evaluate_only:
            ...
            if args.evaluate_only:
                full_experiment_name_original = experiment_name + \
                    "-seed-" + str(args.seed)
                experiment_id_original = sha1(
                    full_experiment_name_original.encode("utf-8")).hexdigest()[:8]
                checkpoint_dir_wandb = os.path.join(
                    fulldir_experiments, "lag-llama", experiment_id_original, "checkpoints")
                file = os.listdir(checkpoint_dir_wandb)[-1]
                if file:
                    ckpt_path = os.path.join(checkpoint_dir_wandb, file)
        ...
        if not args.single_dataset:
            ...
            if args.stratified_sampling:
                if args.stratified_sampling == "series":
                    ...
                    val_weights = dataset_num_series + test_datasets_num_series
            ...
            while batch_size >= 1:
                try:
                    ...
                    batch_size_search_dir = os.path.join(
                        fulldir_batchsize_search, "batch-size-search-" +
                        str(batch_size)
                    )
    ...
    # Evaluate
    evaluation_datasets = args.test_datasets + \
        train_dataset_names if not args.single_dataset else [
            args.single_dataset]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--all_datasets", type=str,
                        nargs="+", default=ALL_DATASETS)
    parser.add_argument("-t", "--test_datasets",
    ...
    parser.add_argument("--use_single_instance_sampler",
                        action="store_true", default=True)

    # Plot forecasts
    parser.add_argument("--plot_test_forecasts",
                        action="store_true", default=True)
    ...
    parser.add_argument("--use_dataset_prediction_length",
                        action="store_true", default=False)
    parser.add_argument("--single_dataset_last_k_percentage", type=float)

    # CosineAnnealingLR
    parser.add_argument("--use_cosine_annealing_lr",
                        action="store_true", default=False)
    parser.add_argument("--cosine_annealing_lr_t_max", type=int, default=10000)
    parser.add_argument("--cosine_annealing_lr_eta_min",
                        type=float, default=1e-2)

    # Distribution output
    parser.add_argument('--distr_output', type=str,
                        default="studentT", choices=["studentT"])
